...
//Function to format the movements date
const formatMovementDate = function (movementDate, locale) {
  const date = new Date(movementDate);

  //Calculate the number of days passed (now - date)
  const calcDaysPassed = (date1, date2) =>
    Math.round(Math.abs(date2 - date1) / (1000 * 60 * 60 * 24));

  //Call the calcDaysPassed function
...
const displayMovements = function (acc, sort = false) {
  ...
  movs.forEach(function (mov, i) {
    ...
    //Function call to format date
    const displayDate = formatMovementDate(acc.movementsDates[i], acc.locale);

    // Function call to format the movements currency
...
const startLogOutTimer = function () {
  const tick = function () {
    ...
    labelTimer.textContent = `${Math.round(min)}:${sec}`;
